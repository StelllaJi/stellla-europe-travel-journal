...
17. 背景音乐必须由用户主动点击开启，右上角可随时静音；不引用第三方受版权保护的音频。
18. 道具显示尽量使用国内玩家熟悉的中文名；当前包含马卡龙、椰林飘香、莫吉托与玛格丽特。
...
| `assets/items/pastry-macaron-cutout-v1.png` | 开心果、覆盆子与香草马卡龙透明道具 | 已完成并替换巴黎布雷斯特 |
| `assets/items/cocktail-pina-colada-cutout-v1.png` | 椰林飘香透明道具 | 已完成并接入 |
| `assets/items/cocktail-mojito-cutout-v1.png` | 莫吉托透明道具 | 已完成并接入 |
| `assets/items/cocktail-margarita-cutout-v1.png` | 玛格丽特透明道具 | 已完成并接入 |
...
用户喜欢法甜，后续可扩展：千层酥、苹果塔、蒙布朗、布列塔尼酥饼、闪电泡芙等。
...
- 收藏页底部可二次确认清空当前浏览器的全部游戏存档；不影响其他设备或访客。
- 右上角音乐按钮播放原创 Web Audio 记忆盒 BGM，不依赖外部音频文件。
- 行囊中已用马卡龙替换巴黎布雷斯特，并将尼格罗尼、古典和曼哈顿分别迁移为椰林飘香、莫吉托和玛格丽特；其余酒名统一中文显示。
