...
16. Collection 指全部 27 张地点明信片记忆，不以 8 张结局门槛作为收藏总数；碎片是独立的辅助系统。
...
- 回忆册弹窗。
- 顶部入口与收藏弹窗统一命名为“我的明信片／我的明信片记忆”，收藏进度为已收藏数 / 27。
...
- 完整文案校对索引位于 `docs/copy-review.md`，UI、便签、道具、碎片和 27 张明信片均有稳定编号。
- 重置本次旅行按钮。
- 首页已引用 `assets/generated/home-room-v1.png`。
- 27 个回忆地点均已有可用插画并接入随机旅行池。
...
此阶段已完成。`js/data.js` 中 27 个地点的 `art` 引用均指向现有文件。
...
> 请打开当前工作区里的 `europe-postcard-game/HANDOFF.md`，完整阅读后再查看 README、memory-map、art-direction、assets/manifest.json 和现有代码。不要重做已完成资产，不要改变已锁定的小狗设计。27 个地点、12 件道具、6 种碎片、隐藏便签和开放式同行邀请均已接入。下一步优先实现房间摆件或更多隐藏事件。所有新插画必须使用 Imagegen，并保持现有角色和绘本风格。

## 真实照片翻面（2026-07-02）

- 27 张保留地点明信片中，26 张已接入真实照片；红灯区背面使用“禁止拍照”的文字说明。
- 明信片详情和收藏回看都支持“翻到那一天／回到插画”的 3D 翻面。
- 横图、竖图均使用完整图居中，并以同图虚化背景补满，不做统一硬裁切。
- 朋友家公开照片由 Imagegen 精确移除了门牌号和姓名铭牌；原图仅留在被 Git 忽略的 `assets/photos/inbox/`。
- 欧尚与金色女神像已从数据、抽取池和收藏总数中删除；旧存档会自动过滤这两个 ID。
- 最近 10 张抽取结果会进入冷却，权重降至正常值的 12%，但仍保留再次抽到的可能。
- 页面实际加载 Imagegen 插画时使用 `assets/optimized/` 中的压缩 JPEG；收藏缩略图启用懒加载。
- 锁定的法甜与鸡尾酒会在准备行囊界面直接显示碎片余额和解锁按钮；成功解锁后原地变为可选卡片，无需绕去回忆册商店。
